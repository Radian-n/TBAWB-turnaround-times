...
#' 
#' @description
#' This isn't the period where The Black and White Box is closed, rather it is the period where the turnaround times are paused until `january_end_day`.
#' 
#' Note: The function returns the dates within *the same year*. So to get a complete shutdown period, the function needs to be called twice - once for the current year, and once for the next year (year + 1).
#' 
#' @param year A single numeric. The year to get the summer closure dates from.
#' @param december_start_day A single numeric. The day of Decmeber when the summer shutdown begins.
#' @param january_end_day A single numeric. The day of January when normal service resumes.
#'
...
get_summer_holiday_dates <- function(year, december_start_day = 28, january_end_day = 5) {
  ...
  jan_holiday_end_date <- bizdays::following(jan_holiday_end_date, "weekend_cal") # Get first non-weekend day
