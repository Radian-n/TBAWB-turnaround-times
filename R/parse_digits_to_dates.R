...
#'
#' This function is designed for the following rows: C-41, B&W, ECN-2, ALT. It is NOT designed for E-6.
#' This function is generally expected to be used directly after [parse_working_days_to_numeric()].
#'
#' Uses the {bizdays} package with a custom calendar definition to calculate the due dates for various lab services.
#' These are calculated from the provided date `current_date`.
#'
#' @param process_row_df A dataframe with a single row and the columns:
#' `Service`, `Develop Only`, `Dev + Scan`, `Prints Add On`.
#' This corresponds to the turnaround time table from The Black and White Box website.
#' Usually the direct output of [parse_working_days_to_numeric()].
...
#'
#' @returns A dataframe.
#' The same as the input dataframe, but with the cells replaced by dates representing the due dates for services.
#' @export
#'
#' @examples
...
parse_digits_to_dates <- function(
  ...
  assertthat::assert_that(
    ...
    msg = "This function is not designed to handle E-6 turnaround times. 
    These are static (e.g 'Processed Wednesday') and should be used with parse_weekdays_to_date()"
  )
  ...
  assertthat::assert_that(
    ...
    msg = glue::glue(
      ...
      Use bizdays::calendars() to check existing calendars, and bizdays::create.calendar() to create one."
    )
  )
  ...
  assertthat::assert_that(
    ...
    msg = glue::glue(
      "`current_date` must be of type Date, not type {typeof(current_date)}."
    )
  )
  assertthat::assert_that(
    length(current_date) == 1,
    msg = glue::glue(
      "`current_date` must be a single date, not a vector of length: {length(current_date)}."
    )
  )
